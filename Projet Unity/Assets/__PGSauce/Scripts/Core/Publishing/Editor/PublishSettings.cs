...
namespace PGSauce.Core.Publishing
{
    ...
    public abstract class PublishSettings : ScriptableObject
    {
        ...
        {
            PGDebug.Message("Setting Game Analytics").Log();
            gameAnalyticsPrivateKey = gameAnalyticsPrivateKey.Trim();
            gameAnalyticsPublicKey = gameAnalyticsPublicKey.Trim();
            if (string.IsNullOrEmpty(gameAnalyticsPrivateKey) || string.IsNullOrEmpty(gameAnalyticsPublicKey))
            {
            ...
            {
                GameAnalytics.SettingsGA.AddPlatform(RuntimePlatform);
            }

            var index = GameAnalytics.SettingsGA.Platforms.IndexOf(RuntimePlatform);
            
            GameAnalytics.SettingsGA.UpdateGameKey(index, gameAnalyticsPublicKey);
            GameAnalytics.SettingsGA.UpdateSecretKey(index, gameAnalyticsPrivateKey);
            GameAnalytics.SettingsGA.Build[index] = Version.CurrentVersion;
